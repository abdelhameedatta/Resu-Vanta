...
function analyzeResume(resume, jobDescription) {
  ...
  const level =
    score < 50 ? 'Low Match' : score < 75 ? 'Medium Match' : 'High Match';
      ...
      <section className="home-section">
        ...
        <div className="service-grid">
          <div className="service-card">
            ...
            <p>
              Upload your current CV and paste the job description. Get a free
          ...
          <div className="service-card featured">
            ...
            <p>
              No CV yet? Answer guided questions and generate an optimized,
  ...
  useEffect(() => {
    ...
    if (payment === 'success' && saved) {
      try {
        ...
        setMessage(
          `Payment confirmed. You can generate up to ${remaining} optimized CV output(s) while this page is open.`
        );
      } catch (e) {
        setError(
        ...
        <div className="paid-output">
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 12, color: '#64748b', marginBottom: 8 }}>
              Write 3–5 sentences about your background, skills, and career
              goal.
            </p>
            <textarea
              style={{ minHeight: 120 }}
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. UAE Pharmacist License, DHA"
                />
              </div>
            </div>
          </div>
...
function BuilderWizard() {
  ...
  function updateBuilder(key, value) {
    setBuilder((old) => ({ ...old, [key]: value }));
  }
  function updateExperience(index, key, value) {
    const updated = [...builder.experiences];
  ...
  function removeExperience(index) {
    if (builder.experiences.length === 1) return;
    setBuilder((old) => ({
      ...old,
      experiences: old.experiences.filter((_, i) => i !== index),
    }));
  }
  function formatExperience() {
    return builder.experiences
      .map((exp) => {
        const r =
  ...
  function generateBuilderCV() {
    const skillKeywords = extractKeywords(
      `${builder.targetJob} ${builder.jobDescription} ${builder.technicalSkills}`,
      10
    );
    const cv = {
      name: builder.name || 'NAME',
      address: builder.address || 'Not provided',
      phone: builder.phone || 'Not provided',
      email: builder.email || 'Not provided',
      linkedin: builder.linkedin || 'Not provided',
      summary: `Motivated professional seeking a ${
        builder.targetJob || 'target'
      } position. This CV has been built and optimized for the target role by highlighting relevant experience, skills, and ATS-friendly keywords.`,
      experience: formatExperience(),
      education: `${builder.degree || 'Degree not provided'}\n${
        builder.university || 'University not provided'
      } | ${builder.graduationYear || 'Year not provided'} | ${
        builder.educationCountry || 'Country not provided'
      }`,
      softSkills:
        builder.softSkills ||
      ...
      technicalSkills:
        ...
        (skillKeywords.length ? skillKeywords.join(', ') : 'Not provided'),
      internshipCourses: `${
        builder.internships || 'Internship not provided'
      }\n${builder.courses || 'Courses not provided'}`,
      additionalInfo:
        builder.additionalInfo ||
        'CV built with ATS-friendly structure and optimized for the target role.',
      language: builder.languages || 'Not provided',
      license: builder.licenses || 'Not provided',
    };
    setBuiltCV(cv);
  }
  async function handleBuilderPayment() {
    setIsPaying(true);
    try {
      await startCheckout('builder', { builder });
    } catch {
      setIsPaying(false);
...
Experience: ${builder.experiences
        .map(
          (e) =>
            `${e.jobTitle} at ${e.company} (${e.startDate}-${e.endDate}): ${e.responsibilities} | Achievements: ${e.achievements}`
        )
        .join(' | ')}
...
Additional: ${builder.additionalInfo}
      ...
      const cv = {
        ...
        summary:
          parsed.summary ||
          `Motivated ${builder.targetJob} professional with experience in ${builder.technicalSkills}.`,
        experience: parsed.experience || formatExperience(),
        education:
          parsed.education ||
          `${builder.degree} - ${builder.university} (${builder.graduationYear})`,
        softSkills:
          parsed.softSkills ||
          builder.softSkills ||
          'Communication, teamwork, problem solving',
        technicalSkills:
          parsed.technicalSkills || builder.technicalSkills || 'Not provided',
        internshipCourses:
          parsed.internshipCourses ||
          `${builder.internships} ${builder.courses}`,
    ...
    <div className="builder-wizard">
      ...
      {step === 1 && (
        <div className="wizard-card">
          ...
          <input
            ...
            value={builder.targetJob}
            onChange={(e) => updateBuilder('targetJob', e.target.value)}
          />
          <textarea
            placeholder="Optional: Paste Job Description"
            value={builder.jobDescription}
            onChange={(e) => updateBuilder('jobDescription', e.target.value)}
          />
        </div>
      )}
        ...
        <div className="wizard-card">
          ...
          <div className="form">
            ...
            <input
              ...
              onChange={(e) => updateBuilder('address', e.target.value)}
            />
            <input
              placeholder="Phone"
              value={builder.phone}
        ...
        <div className="wizard-card">
          ...
          <div className="form">
            ...
            <input
              ...
              value={builder.university}
              onChange={(e) => updateBuilder('university', e.target.value)}
            />
            <input
            ...
            <input
              placeholder="Country"
              value={builder.educationCountry}
              onChange={(e) =>
                updateBuilder('educationCountry', e.target.value)
              }
            />
          </div>
        </div>
      ...
      {step === 4 && (
        <div className="wizard-card">
          ...
          <button className="secondary" onClick={addExperience}>
      ...
      {step === 5 && (
        <div className="wizard-card">
          ...
          <textarea
            ...
            value={builder.softSkills}
            onChange={(e) => updateBuilder('softSkills', e.target.value)}
          />
          <textarea
      ...
      {step === 6 && (
        <div className="wizard-card">
          <h3>Internship and Courses</h3>
          <textarea
            placeholder="Internships"
            value={builder.internships}
            onChange={(e) => updateBuilder('internships', e.target.value)}
          />
          <textarea
            placeholder="Courses / Training"
            value={builder.courses}
            onChange={(e) => updateBuilder('courses', e.target.value)}
          />
        </div>
      )}
      ...
      {step === 7 && (
        <div className="wizard-card">
          <h3>Languages, Licenses and Additional Info</h3>
          <textarea
            placeholder="Languages"
            value={builder.languages}
            onChange={(e) => updateBuilder('languages', e.target.value)}
          />
          <textarea
            placeholder="Licenses"
            value={builder.licenses}
            onChange={(e) => updateBuilder('licenses', e.target.value)}
          />
          <textarea
            placeholder="Additional Information"
            value={builder.additionalInfo}
            onChange={(e) => updateBuilder('additionalInfo', e.target.value)}
          />
        </div>
      )}
              ...
              <>
                <button onClick={generatePaidBuilderCV} disabled={isGenerating}>
                  {isGenerating
                    ...
                    : 'Generate Paid CV Output'}
                </button>
                <p className="success">
                  Payment confirmed. Remaining outputs: {remainingOutputs}
                </p>
              </>
            ) : showBuilderPayment ? (
              <StripeWrapper
                ...
                onSuccess={() => {
                  ...
                  setPaymentConfirmed(true);
                  setRemainingOutputs(3);
                  sessionStorage.setItem('resuvanta_paid_builder', 'true');
                  sessionStorage.setItem('resuvanta_outputs_builder', '3');
                }}
...
function BuilderPage() {
  return (
    <section className="section">
      ...
      <p className="muted">
        No CV yet? Answer simple questions, add your target job, and generate an
        optimized ATS-friendly CV.
      </p>
      <StepsStrip
        steps={[
          'Enter target job',
      ...
      <div className="builder-price-box">
        ...
        <p>
          Build a complete CV from scratch and optimize it for your target job
          in one package. Final output: PDF only.
        </p>
      </div>
      <BuilderWizard />
    </section>
...
function LinkedInPage() {
  ...
  async function generateFullLinkedInOptimization() {
    ...
    try {
      ...
      setFullLinkedInOutput({
        headline:
          parsed.headline ||
          `${targetRole} | ${experience} | Open to New Opportunities`,
        about:
          parsed.about || `Results-focused ${targetRole} with ${experience}.`,
        skills:
          parsed.skills ||
          `${targetRole}, Communication, Problem Solving, Teamwork`,
        recruiterKeywords:
          parsed.recruiterKeywords || `${targetRole}, ${experience}`,
      });
    } catch (e: any) {
  ...
  return (
    <section className="section">
      ...
      <p className="muted">
        Free preview gives one headline only. Full LinkedIn optimization unlocks
        About section, experience rewrite, skills, and recruiter keywords.
      </p>
      <StepsStrip
        steps={[
          'Enter target role',
      ...
      <div className="form">
        ...
        />
        <input
          placeholder="Short experience, e.g. 5+ years pharmacy experience"
          value={experience}
          onChange={(e) => setExperience(e.target.value)}
        />
      </div>
      <div className="button-row">
        <button onClick={generateHeadlinePreview}>
          Generate Free Headline Preview
        </button>
      </div>
      {headline && (
        <div className="preview-box single">
          ...
          <div className="locked-card">
            ...
            <p>
              Get your professional headline, About section, experience wording,
              skills list, and recruiter search keywords.
            </p>
            {paymentConfirmed ? (
              <button
                onClick={generateFullLinkedInOptimization}
                disabled={isGeneratingLinkedIn}
              >
                {isGeneratingLinkedIn
                  ? '⏳ AI is optimizing your profile...'
                  : 'Generate Full LinkedIn Optimization'}
              </button>
            ) : showLinkedInPayment ? (
              <StripeWrapper
                service="linkedin"
                onSuccess={() => {
                  ...
                  setPaymentConfirmed(true);
                  sessionStorage.setItem('resuvanta_paid_linkedin', 'true');
                }}
      ...
      {fullLinkedInOutput && (
        <div className="paid-output">
          ...
          <p>{fullLinkedInOutput.headline}</p>
          <h4>About Section</h4>
          <p>{fullLinkedInOutput.about}</p>
          <h4>Skills Keywords</h4>
          <p>{fullLinkedInOutput.skills}</p>
          <h4>Recruiter Search Keywords</h4>
          <p>{fullLinkedInOutput.recruiterKeywords}</p>
          <button
            onClick={() => {
    ...
    <section className="section">
      ...
      <div className="pricing-grid">
        <div className="price-card">
          ...
          <h2>{PRICES.optimization}</h2>
          <p>
            Improve your existing CV with job-focused recommendations, stronger
            wording, better structure, and ATS-friendly keyword guidance.
          </p>
          <ul>
            <li>Free resume preview</li>
            <li>ATS keyword analysis</li>
            <li>Content and wording improvements</li>
            <li>Role-focused optimization tips</li>
            <li>Clear strengths and weaknesses report</li>
          </ul>
        </div>
        <div className="price-card featured">
          ...
          <h2>{PRICES.builder}</h2>
          <p>
            Create a professional CV from scratch with guided sections,
            optimized wording, and a structure designed for job applications.
          </p>
          <ul>
            <li>Step-by-step CV builder</li>
            <li>Professional CV structure</li>
            <li>ATS-friendly wording</li>
            <li>Skills and experience improvement</li>
            <li>Complete resume package guidance</li>
          </ul>
        </div>
        <div className="price-card">
          <h3>LinkedIn Optimization</h3>
          <h2>{PRICES.linkedin}</h2>
          <p>
            Improve your LinkedIn profile so recruiters can understand your
            value faster and find you through better keywords.
          </p>
          <ul>
            <li>Professional headline improvement</li>
            <li>About section rewrite guidance</li>
            <li>Skills keyword suggestions</li>
            <li>Recruiter search optimization</li>
            <li>Profile visibility improvement tips</li>
          </ul>
        </div>
      </div>
    </section>
...
function FAQPage() {
  return (
    <section className="section">
      <h2>FAQ</h2>
      <details>
        <summary>Does the free preview generate the full CV?</summary>
        <p>
          No. The free preview shows match score bars, missing keyword count,
          and one improvement suggestion. Full keywords and professional
          sentences are revealed after payment.
        </p>
      </details>
      <details>
        <summary>What files can users upload?</summary>
        <p>PDF, DOCX, and TXT are supported.</p>
      </details>
      <details>
        <summary>What file does the user download?</summary>
        <p>The final output is PDF only.</p>
      </details>
      <details>
        <summary>Does this guarantee a job?</summary>
        <p>
          No. The tool helps improve CV quality and job matching, but it does
          not guarantee interviews or job offers.
        </p>
      </details>
    </section>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (
      ['optimization', 'builder', 'linkedin', 'pricing', 'faq'].includes(
        selectedPage
      )
    )
  ...
  return (
    <div className={darkMode ? 'site dark' : 'site'}>
      ...
      <footer>
        <p>
          ResuVanta provides automated CV optimization support only. It does not
          guarantee interviews, job offers, or hiring decisions.
        </p>
      </footer>
    </div>
  );
