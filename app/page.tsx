...
function openPDFWindow(cv, title='Optimized CV') {
  const html = `<html><head><title>${escapeHTML(title)}</title>
    ...
    <h2>SKILLS</h2><p><b>Soft Skills:</b> ${escapeHTML(cv.softSkills)}</p><p><b>Technical Skills:</b> ${escapeHTML(cv.technicalSkills)}</p>
            ...
            {paymentConfirmed ? (
              ...
              <StripeWrapper
                ...
                onSuccess={() => {
                  ...
                  setMessage('Payment confirmed. You can generate up to 3 optimized CVs while this page is open.');
                }}
